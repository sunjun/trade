...
信号逻辑：
  多头入场：收盘价 < VWAP - std_mult × σ 且 RSI < oversold（超卖+VWAP低估）
  多头出场：收盘价回到 VWAP 或触及止损
  空头入场（仅合约）：收盘价 > VWAP + std_mult × σ 且 RSI > overbought
  空头出场：收盘价回到 VWAP 或触及止损

VWAP：每日 UTC 00:00 重置，σ 为价格相对 VWAP 的成交量加权标准差
...
class VwapStrategy(BaseStrategy):
    def __init__(
        ...
        std_mult = config.get("vwap_std_mult", 1.5)
        rsi_period = config.get("rsi_period", 14)
        atr_period = config.get("atr_period", 14)

        self._vwap = RunningVWAP(std_mult)
        self._rsi = RunningRSI(rsi_period)
        self._atr = RunningATR(atr_period)
        self._sl_mult: float = config.get("atr_sl_multiplier", 1.5)
        self._oversold: float = config.get("rsi_oversold", 35)
        self._overbought: float = config.get("rsi_overbought", 65)
    ...
    async def on_candle(self, candle: Candle) -> list[Signal]:
        ...
        if self._last_date is not None and candle_date != self._last_date:
            logger.info(f"[{self.name}] New day {candle_date}, resetting VWAP")
            self._vwap.reset()
        self._last_date = candle_date

        close = candle.close
        self._vwap.update(close, candle.volume)
        ...
        upper = self._vwap.upper
        lower = self._vwap.lower
        ...
        # ── 入场 ───────────────────────────────────────────────────────────────
        if self._state.flat:
            if close < lower and rsi < self._oversold and cooldown_ok:
                ...
                signals.append(Signal(
                    ...
                    stop_loss=sl,
                    reason=f"VWAP lower breach + RSI={rsi:.1f}<{self._oversold} | SL={sl:.4f}",
                ))
                self._state.open(PosSide.LONG, close, sl)
                self._candles_since_trade = 0
                logger.info(f"[{self.name}] LONG ENTRY @ {close:.4f} (VWAP={vwap:.4f})")

            elif self._can_short and close > upper and rsi > self._overbought and cooldown_ok:
                sl = close + self._sl_mult * atr
                signals.append(Signal(
                    inst_id=self.symbol,
                    side=OrderSide.SELL,
                    ...
                    stop_loss=sl,
                    reason=f"VWAP upper breach + RSI={rsi:.1f}>{self._overbought} | SL={sl:.4f}",
                ))
                self._state.open(PosSide.SHORT, close, sl)
                self._candles_since_trade = 0
                logger.info(f"[{self.name}] SHORT ENTRY @ {close:.4f} (VWAP={vwap:.4f})")

        # ── 出场：价格回到 VWAP ────────────────────────────────────────────────
        else:
            if self._state.pos_side == PosSide.LONG and close >= vwap:
                sig = self._build_close(close, f"Price returned to VWAP={vwap:.4f}")
                if sig:
                    signals.append(sig)
                    self._state.close()
                    self._candles_since_trade = 0
            ...
            elif self._state.pos_side == PosSide.SHORT and close <= vwap:
                sig = self._build_close(close, f"Price returned to VWAP={vwap:.4f}")
                if sig:
                    signals.append(sig)
                    self._state.close()
