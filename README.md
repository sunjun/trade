...
> 调大 `--max-bars` 时缓存会自动向历史回补，不必手动删缓存。

`--capital` 请填**真实权益**：仓位是按权益百分比反推的，用默认 10000 跑出来的
张数比用 800 大一个量级，只有按真实资金跑才会暴露「张数被交易所 `minSz` 取整
吃掉」这类小资金特有的问题。

历史深度受 OKX 接口限制（15m 约 6 个月、1H 约 1.5 年、4H 约 5 年）。实际拿到的
根数少于请求时会打 WARNING，**别把「跑满了 6 个月」和「请求了 17000 根」当成
一回事** —— 尤其是评估马丁型策略的左尾，样本区间不够长的结论没有意义。
...
启动时若交易所上已存在该品种持仓，引擎按策略配置的 `on_existing_position` 处理：
...
- **保证金模式必须对上。** OKX 用 `instId + posSide + mgnMode` 三元组标识持仓，
  同品种同方向的全仓和逐仓是**两笔互不相干的仓位**。策略只会接管与自己
  `td_mode` 一致的那笔，其余的只告警不碰。用全仓的策略去平一笔逐仓持仓，
  交易所会回 `sCode 51169`「该方向无持仓」。
- **合并持仓只平自己那份。** 同方向同保证金模式的仓位在交易所侧会合并成一笔，
  策略平仓时只会平掉自己成交累计出来的张数，不会动手动仓或其他策略的仓位。
...
另有四个**默认关闭**的可选闸门（回测未显示稳定收益，只留开关，勿盲目启用）：

| 配置 | 作用 |
|---|---|
| `drop_atr_min` | 首仓要求距近期高点跌够 N 个 ATR。用波动率归一化——固定百分比在 ATR=2% 和 ATR=6% 的时期含义完全不同 |
| `require_reclaim` | 首仓要求右侧确认：本根收阳**且**收在上一根高点之上。超跌判据本身不含「跌势停没停」的信息，单用会在下跌全程接飞刀 |
| `ladder_spread` | 加仓档位均匀铺满整个支撑区间，末档落在前低、紧挨止损 |
| `trend_max_margin` | 快线领先慢线的上限，过滤「刚跑完一段」的动能衰竭位 |
