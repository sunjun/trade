...
| 日志行 | 含义 |
|---|---|
| `⏳ ETH-USDT-SWAP 最新价 1889.5 (本根未收盘 …)` | 行情心跳，最多每 60 秒一条，证明数据链路是通的 |
| `📊 收盘K线 ETH-USDT-SWAP 15m O= H= L= C= V=` | 一根 K 线收盘，策略在这一刻做决策 |
| `本根K线不交易｜<原因>` | **不下单的具体原因**（见下表） |
| `>>> Signal: BUY …` → `下单中：BUY 4 张 … ≈ 191.00 USDT 名义` → `Order PLACED ✓` | 信号 → 下单 → 交易所受理 |
| `📥 订单回报 … 状态=filled 成交 4@1889.5` | 交易所推回的成交回报 |
| `📈 持仓 ETH-USDT-SWAP long 4 张 浮动盈亏 +2.31 USDT` | 持仓快照，按品种每 60 秒一条 |
| `💰 账户刷新：权益 800.00 USDT｜单品种名义上限 200.00 USDT (25%)` | 每 60 秒的账户与闸门状态 |
