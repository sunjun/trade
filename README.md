...
运行时每根 K 线都会打印行情与决策依据，不用再靠猜。
日志时间戳统一显示**东八区**（内部存储仍是 UTC）：

| 日志行 | 含义 | 频率 |
|---|---|---|
| `⏳ ETH-USDT-SWAP 最新价 1889.5 (本根未收盘 …)` | 行情心跳，证明数据链路是通的 | 每根 K 线一条（15m 策略 = 15 分钟） |
| `📊 收盘K线 ETH-USDT-SWAP 15m O= H= L= C= V=` | 一根 K 线收盘，**策略只在这一刻做决策** | 每根 K 线 |
| `本根K线不交易｜<原因>` | **不下单的具体原因**（见下表） | 每根 K 线 |
| `>>> Signal: BUY …` → `下单中：BUY 4 张 … ≈ 191.00 USDT 名义` → `Order PLACED ✓` | 信号 → 下单 → 交易所受理 | 有信号时 |
| `📥 订单回报 … 状态=filled 成交 4@1889.5` | 交易所推回的成交回报 | 每次回报 |
| `📈 持仓 ETH-USDT-SWAP long 4 张 浮动盈亏 +2.31 USDT` | 持仓快照，按品种节流 | 每小时 |
| `💰 账户：权益 800.00 USDT｜单品种名义上限 200.00 USDT (25%)` | 账户与闸门状态 | 每小时 |

> 持仓/账户**每 60 秒仍会通过 REST 全量刷新**（风控熔断、持仓对齐都依赖它），
> 只是 INFO 打印节流到一小时一条；每分钟的刷新记录在 DEBUG 级别，进 `logs/` 文件日志。

金字塔策略的「不交易原因」会精确到差多少：

```
本根K线不交易｜空仓等待｜趋势过滤未通过：4H EMA21=1881.2 ≤ EMA55=1902.7（差 21.5，需快线上穿）
本根K线不交易｜空仓等待｜趋势向上，但现价 1975.3 高于第一道支撑 1944.3，还需回调 1.57% 才建首仓
本根K线不交易｜持仓 2/6 档 0.18 张｜均价 1930.4 现价 1921.0 (-0.49%)｜止盈 2003.4 (+3.8%) 止损 1766.9｜下一档(3/6)需先跌至支撑 1897.0（还差 1.27%）
本根K线不交易｜持仓 3/6 档 …｜已触及支撑 1875.8，但距上次成交 1897.0 只回落 21.2 < 最小间隔 24.3 (ATR 20.3 × 1.2)，跌到 1872.7 才加仓
```

信号产生了却没下成单，看这两类：

- `Signal BLOCKED by risk: …` — 被熔断/暂停/频率限制拦下
- `信号被跳过：可下张数为 0（策略算出 X，闸门后 Y）｜…｜交易所最小下单量 minSz=…` — 资金不足、
  低于交易所最小下单量，或撞上 `RISK__MAX_POSITION_PCT` 名义上限
- `张数被全局仓位闸门截断：X → Y` — 下单量被 `max_position_pct` 削减（仍会下单，只是变小）

想看更细的指标数值（EMA/MACD/ATR 逐根），把 `LOG_LEVEL=DEBUG` 写进 `.env`——
`logs/` 里的文件日志本来就是 DEBUG 级别，终端才受 `LOG_LEVEL` 控制。
